...
class ReportController extends Controller
{
    ...
    public function commissionsDeposit()
    {
        ...
        $logs = CommissionLog::where('type','deposit')->with(['user:username,id','bywho:username,id'])->latest()->paginate(getPaginate());
    ...
    public function commissionsInvest()
    {
        ...
        $logs = CommissionLog::where('type','invest')->with(['user:username,id','bywho:username,id'])->latest()->paginate(getPaginate());
        $empty_message = 'No log.';
        return view('admin.reports.commission-log', compact('page_title', 'logs', 'empty_message'));
    }
    ...
    public function commissionsInterest()
    {
        ...
        $logs = CommissionLog::where('type','interest')->with(['user:username,id','bywho:username,id'])->latest()->paginate(getPaginate());
        $empty_message = 'No log.';
        return view('admin.reports.commission-log', compact('page_title', 'logs', 'empty_message'));
    }
    ...
    public function commissionsSearch(Request $request)
    {
        ...
        $logs = CommissionLog::whereHas('user', function ($user) use ($search) {
                $user->where('username', 'like',"%$search%");
            })->orWhere('trx', $search)->with(['user:username,id','bywho:username,id'])->latest()->paginate(getPaginate());
        $empty_message = 'No log.';
