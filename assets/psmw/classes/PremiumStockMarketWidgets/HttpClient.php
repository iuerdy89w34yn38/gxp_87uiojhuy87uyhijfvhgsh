...
* https://github.com/rgamba/httpclient
* ---
...
class HttpClient{
    ...
    public $host;
    public $port=80;
    public $user_agent="Mozilla/5.0 (Macintosh; Intel Mac OS X 10.9; rv:32.0) Gecko/20100101 Firefox/32.0";
        ...
        while(!feof($this->socket)){
            ...
            if($in_head)
                ...
                if(isset($this->response_headers['Transfer-Encoding']) && strtolower($this->response_headers['Transfer-Encoding'])=="chunked"){
                    if(preg_match('/^[0-9a-f]+$/i',trim($line))==1){
                        continue;
                    }    
                }
                $body.=$line;       
